...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
			...
			error_log("Uploaded file: $upload");
			error_log("Directory path: $dir_path");
	
			$allowed = array('image/png','image/jpeg');
	
			if(!in_array($type, $allowed)){
				$resp['msg'] = "Image upload failed: invalid file type ($type).";
			} else {
				$new_height = 200; 
				$new_width = 200; 
				...
				if($gdImg){
					...
					if($uploaded_img){
						...
						$resp['msg'] = "Image upload failed: unable to save resized image.";
					}
				} else {
					$resp['msg'] = "Image upload failed: unable to create image resource.";
				}
			}
			if(isset($resp['msg'])) {
